...
def gain(choix_joueur1, choix_joueur2):
    ...
    * 0 en cas d'égalité
    ...
    if choix_joueur1 == choix_joueur2:
        gain = 0
    # remarque : on peut fractionner l'écriture d'une commande sur
    # plusieurs lignes en utilisant le caractère \ 
...
def jouer(choix):
    ...
    if (abs(score) < 3):
        ordi = tirage()
        score = score + gain(choix, ordi)
        print(f"Score : {score}")
    if abs(score) < 3:
        conclusion = "partie en cours...\n"
    elif score == 3:
        conclusion = "Vous avez gagné !"
    else:
        conclusion = "L'ordinateur a gagné !"
    print(conclusion)
